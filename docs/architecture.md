...
*Status:* this flow works end to end over the binary
[frame format](protocol.md), including `CANCEL` and `ERROR`. Still **one
in-flight request per Agent** (a second returns `503`) with bodies
**buffered** — Stage 4 adds multiplexing, Stage 5 makes it stream.
